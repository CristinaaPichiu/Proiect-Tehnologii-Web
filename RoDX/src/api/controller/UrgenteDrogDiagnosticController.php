...
class UrgenteDrogDiagnosticController {
    ...
    {
        $sqlQuery = sprintf("SELECT s.cauza, s.an, (s.canabis + s.stimulanti + s.opiacee + s.nsp) as droguri FROM %s s order by s.cauza", $this->db_table);
    ...
    {
        $sqlQuery = sprintf("SELECT cauza, an, canabis FROM %s s order by cauza", $this->db_table);
        $stmt = $this->conn->prepare($sqlQuery);
        $stmt->execute();
        return $stmt;
    }
    ...
    {
        $sqlQuery = sprintf("SELECT cauza, stimulanti, an FROM %s s order by an", $this->db_table);
        $stmt = $this->conn->prepare($sqlQuery);
        $stmt->execute();
        return $stmt;
    }
    ...
    {
        $sqlQuery = sprintf("SELECT s.cauza, (s.canabis + s.stimulanti + s.opiacee + s.nsp) FROM %s s WHERE an=?", $this->db_table);
        $stmt = $this->conn->prepare($sqlQuery);
