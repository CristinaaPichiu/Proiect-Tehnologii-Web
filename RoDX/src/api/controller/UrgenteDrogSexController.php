...
class UrgenteDrogSexController {
    ...
    {
        $sqlQuery = sprintf("SELECT s.sex, s.an, (s.canabis + s.stimulanti + s.opiacee + s.nsp) as droguri FROM %s s order by sex", $this->db_table);
    ...
    {
        $sqlQuery = sprintf("SELECT sex, an, canabis FROM %s s order by sex", $this->db_table);
        $stmt = $this->conn->prepare($sqlQuery);
        $stmt->execute();
        return $stmt;
    }
    ...
    {
        $sqlQuery = sprintf("SELECT sex, (s.canabis + s.stimulanti + s.opiacee + s.nsp) as droguri FROM %s s WHERE an=?", $this->db_table);
        $stmt = $this->conn->prepare($sqlQuery);
